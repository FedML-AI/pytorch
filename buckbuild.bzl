...
def read_bool(section, field, default, required = True):
    ...
    if val != None:
        if val in ["0", "1"]:
            ...
            fail(
                "`{}:{}`: must be one of (0, 1), but was {!r}".format(section, field, val),
            )
    elif default != None:
        return default
    elif not required:
...
C10_DEP = "//c10:c10" if is_oss_build() else "//xplat/caffe2/c10:c10"
...
def define_buck_targets(
    ...
    [
        ...
        ) for enable_flatbuffer in ([False] if is_oss_build else [True, False])
    ]
    ...
    [
        pt_xplat_cxx_library(
            ...
            deps = [
                ...
                ":torch_headers",
            ] + ([] if is_oss_build else [":torch_kineto_profiling"]),
            exported_deps = [
                ":aten_cpu",
                ":torch_core_pickle_and_flatbuffer" if enable_flatbuffer else ":torch_core",
                C10_DEP,
